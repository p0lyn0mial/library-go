...
func TestEncryptionIntegration(tt *testing.T) {
	...
	require.NoError(t, err)
	kmsKeyState, err := secrets.ToKeyState(kmsKeySecret)
	require.NoError(t, err)
	require.NotNil(t, kmsKeyState.KMSConfig, "expected KMSConfig to be present in key state")
	require.NotNil(t, kmsKeyState.KMSConfig.Provider, "expected KMS provider config to be present in key state")
	require.Equal(t, configv1.VaultKMSProvider, kmsKeyState.KMSConfig.Provider.Type)
	require.Equal(t, "https://vault.example.com", kmsKeyState.KMSConfig.Provider.Vault.VaultAddress)
	require.Equal(t, "test-transit-key", kmsKeyState.KMSConfig.Provider.Vault.TransitKey)
