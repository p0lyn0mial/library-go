...
func ToKeyState(s *corev1.Secret) (state.KeyState, error) {
	...
	case state.KMS:
		...
		if v, ok := s.Data[EncryptionSecretKMSEncryptionConfig]; ok && len(v) > 0 {
			kmsConfiguration, err := codec.DecodeKMSConfiguration(v)
			if err != nil {
		...
		if v, ok := s.Data[EncryptionSecretKMSProviderConfig]; ok && len(v) > 0 {
			kmsConfig, err := codec.DecodeKMSConfig(v)
			if err != nil {
...
func FromKeyState(component string, ks state.KeyState) (*corev1.Secret, error) {
	...
	if ks.HasKMSEncryption() {
		encryptionConfigurationData, err := codec.EncodeKMSConfiguration(ks.KMSConfig.Encryption)
		if err != nil {
			return nil, err
		}
	...
	if ks.HasKMSProvider() {
		providerData, err := codec.EncodeKMSConfig(ks.KMSConfig.Provider)
		if err != nil {
			return nil, err
		}
