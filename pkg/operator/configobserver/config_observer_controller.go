...
func (c ConfigObserver) updateObservedConfig(syncCtx factory.SyncContext, existingConfig map[string]interface{}, mergedObservedConfig map[string]interface{}) error {
	if len(c.nestedConfigPath) == 0 {
		if !equality.Semantic.DeepEqual(existingConfig, mergedObservedConfig) {
			...
			return c.updateConfig(syncCtx, mergedObservedConfig)
		}
		return nil
	}
	...
	if !equality.Semantic.DeepEqual(existingConfigNested, mergedObservedConfigNested) {
		...
		if err := unstructured.SetNestedField(updatedConfig, mergedObservedConfigNested, c.nestedConfigPath...); err != nil {
			syncCtx.Recorder().Warningf("ObservedConfigWriteError", "Failed to write nested (%q) observed config: %v", strings.Join(c.nestedConfigPath, "/"), err)
			return fmt.Errorf("error writing nested observed config: %v", err)
		}
		return c.updateConfig(syncCtx, updatedConfig)
	}
	return nil
}

func (c ConfigObserver) updateConfig(syncCtx factory.SyncContext, config map[string]interface{}) error {
	if _, _, err := v1helpers.UpdateSpec(c.operatorClient, v1helpers.UpdateObservedConfigFn(config)); err != nil {
